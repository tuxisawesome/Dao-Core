...
def init(isrecovery=False,nofsck=False,path="etc/"):
    mods = configmgr.readconfig("modules.cfg",path)
    init = configmgr.readconfig("init.cfg",path)
    config = configmgr.readconfig("config.cfg",path)
    recovery = configmgr.readconfig("recovery.cfg",path)
    ...
    if bootr or isrecovery:
        ...
        start.init(display,init,verbosedrivers,configmgr,drivermgr,drivers,drivernames,nofsck)
    '''
...
class configmgr:
    ...
    def writeconfig(file, config,path="etc/"):
